...
async def create_expense(
    expense_form_data: ExpenseRequest, expense_service: ExpenseService = Depends()
):
    if not expense_service.user_service.get_user_by_id(expense_form_data.paid_by_id):
        raise HTTPException(status_code=404, detail="User not found")

    if not expense_service.expense_category_service.get_expense_category_by_id(
        expense_form_data.expense_category_id
    ):
...
async def update_expense(
    expense_id: int,
    expense_form_data: ExpenseRequest,
